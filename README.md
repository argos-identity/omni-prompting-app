...
> **정책 문서 → 2단계 LLM 파이프라인 → workflow.md 생성**

1. 정책 문서 업로드 (PDF, DOCX, TXT, MD) 또는 **URL 입력**
2. **1차 LLM (Policy Extractor)**: Claude가 정책 데이터 추출 (summary, validationRules, structuredContent)
3. **2차 LLM (Workflow Generator)**: 추출된 데이터를 meta-prompt에 주입하여 workflow.md 생성
4. 생성된 워크플로우 복사/다운로드
...
[정책 문서]
    ...
    ↓
[추출된 텍스트]
    ↓ 1차 LLM (Policy Extractor)
    ↓ - Max Tokens: 8,192
[추출된 데이터: summary, validationRules, structuredContent]
    ↓ Meta Prompt 템플릿 치환
...
- 확대/축소 토글 기능
- 파일 기반 프롬프트 로드 (`/prompt/policy-prompt/*.md`)
...
| `api/generate/route.ts` | 워크플로우 생성 API (파일/URL 처리 → LLM 파이프라인) |
...
| 디렉토리 | 파일 | 설명 |
|----------|------|------|
| `policy-prompt/` | `system-prompt.md` | Omni Verification Agent 시스템 정의 (v2.0) |
| `policy-prompt/` | `meta-prompt.md` | workflow.md 생성용 메타 프롬프트 (v7.0) |
| `action-prompt/` | `system-prompt.md` | 액션 워크플로우용 시스템 정의 (v1.0.0, 개발 중) |

### `/types` - TypeScript 타입
| 파일 | 설명 |
|------|------|
| `workflow.ts` | GeneratedWorkflow, ExtractedPolicyData, WorkflowAction |
...
│   └── utils/              # 공통 유틸리티
├── prompt/                 # 프롬프트 템플릿
│   ├── policy-prompt/      # 정책 분석용 프롬프트
│   │   ├── system-prompt.md (v2.0)
│   │   └── meta-prompt.md (v7.0)
│   └── action-prompt/      # 액션 워크플로우용 (개발 중)
│       └── system-prompt.md (v1.0.0)
...
sequenceDiagram
    ...
    API->>PolicyExtractor: extractPolicyData()
    PolicyExtractor->>Claude: 1차 LLM 호출 (8K tokens)
    Claude-->>PolicyExtractor: 추출된 데이터 (JSON)
    PolicyExtractor-->>API: { summary, validationRules, content }

    API->>WorkflowGenerator: generateWorkflow()
    WorkflowGenerator->>WorkflowGenerator: Meta Prompt 템플릿 치환
    WorkflowGenerator->>Claude: 2차 LLM 호출 (16K tokens)
...
```json
{
  ...
  "workflow": {
    "actions": [
      {
        "id": "1.0",
        "action_name": "verify_document_authenticity",
        "category": "Document Verification",
        "description": "문서 진위 여부 검증",
        "engine_required": true,
        "engine_type": "OCR",
        "reference_notes": ["정책 문서 3.2절 참조"]
      }
    ],
    "rawContent": "# workflow.md 전체 내용...",
    "workflowMd": "# workflow.md 마크다운 섹션...",
...
### Meta-Prompt
- **v7.0** (현재): JSON 기반 action_workflow, action_flag 분류 체계
- v5.0: XML 섹션 구조 도입
- v3.0: 초기 workflow.md 템플릿

### System-Prompt
- **v2.0** (현재): ZERO HALLUCINATION, 확실성 라벨, 위험 점수 프로토콜
- v1.0: 초기 시스템 정의
